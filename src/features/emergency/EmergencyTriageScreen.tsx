...
export default function EmergencyTriageScreen() {
  ...
  const [cases, setCases] = useState<EmergencyCase[]>(() => {
    const saved = localStorage.getItem(`hms-emergency-cases-${tenant || "default"}`);
    return saved ? JSON.parse(saved) : [];
  });
  const [searchQuery, setSearchQuery] = useState("");

  const saveCases = (nextCases: EmergencyCase[]) => {
    setCases(nextCases);
    localStorage.setItem(`hms-emergency-cases-${tenant || "default"}`, JSON.stringify(nextCases));
  };
  ...
  const handleTransferSuccess = (transferData: any) => {
    const next = cases.map((c) =>
      c.id === transferData.caseId
        ? {
            ...c,
            status: "TRANSFERRED_ICU" as const,
      ...
      {activeTab === "board" && (
        <Card style={{ borderRadius: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 16 }}>
            {filteredCases.filter(c => c.status === "IN_TRAUMA_BAY").length === 0 ? (
              <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "48px 24px", background: "var(--wash-a)", borderRadius: 14, border: "1px dashed var(--line)" }}>
                ...
                <strong style={{ fontSize: 16, color: "var(--ink)", display: "block", marginBottom: 6 }}>
                  No Active Emergency Casualty Cases
                </strong>
                <span style={{ fontSize: 13, color: "var(--slate)", maxWidth: 440, margin: "0 auto 18px", display: "block" }}>
                  All casualty bays are currently clear. Incoming accident victims or trauma cases triaged via Rapid Intake will appear here in real time.
                </span>
                <Button type="button" onClick={() => setIntakeModalOpen(true)} style={{ background: "#DC2626", color: "#fff", fontWeight: 800 }}>
                  ⚡ Rapid Emergency Intake (30s)
                </Button>
              </div>
            ) : (
        ...
        <Card style={{ borderRadius: 16 }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {cases.filter(c => c.status.startsWith("TRANSFERRED")).length === 0 ? (
                  <tr>
                    <td colSpan={5} style={{ textAlign: "center", padding: "32px 14px", color: "var(--slate)", fontStyle: "italic" }}>
                      No patients currently escalated to ICU or OT.
                    </td>
                  </tr>
                ) : (
